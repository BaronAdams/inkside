...
import { Dropdown, Loading } from 'react-daisyui'
import { logOut } from '@/lib/actions/navigation'
...
const Header = ( { session } : { session : Session | null } ) => {
   ...
   useEffect(() => {
      console.log("Session user has changed")
                           ...
                           <div className='flex gap-6 items-center z-30'>
                              <div className="dropdown z-30" role='listbox'>
                                 <div className="group hover:cursor-pointer" tabIndex={0}>
                                    {user?.image ? (<img src={user?.image} className='w-10 h-10 object-cover object-center rounded-full' alt={user?.name + ' image'} />) : (<div className='w-10 h-10 flex justify-center items-center bg-orange-600 text-white font-bold rounded-full duration-300 border-[3.3px] border-[#ea5a0c] group-hover:border-[#ffffff46]'><p className='text-[20px]'>{user?.name?.split(' ')[0][0]}</p></div>)}
                                 </div>
                                 <ul tabIndex={0} className='dropdown-content z-30 menu p-2 shadow bg-base-100 rounded-box w-[170px] mr-[60px]' role='menu' >
